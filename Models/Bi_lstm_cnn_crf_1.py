...
#                                              dtype=tf.float32)
        ...
        with tf.variable_scope('bi-lstm_network'):
            ...
            #hidden_units = 200
            forward_cell = tf.contrib.rnn.LSTMCell(num_units=hidden_uni)
            backward_cell = tf.contrib.rnn.LSTMCell(num_units=hidden_uni)
...
#         self.accuracy_calculation = {'accuracy_calculation': accuracy_calculation}
    ...
    def train(self,sess):
        ...
        if os.path.exists('Modelbilstm/model.ckpt.meta'): 
            saver = tf.train.Saver()
            saver.restore(sess, './Modelbilstm/model.ckpt')
        else:
...
#                         10, data_d['predicted_labels'], batch_labels, batch_sequence_lengths)
        ...
        saver = tf.train.Saver()
        saver.save(sess, 'Modelbilstm/model.ckpt')
